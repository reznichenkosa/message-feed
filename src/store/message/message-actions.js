...
export const OLD_MESSAGES_FETCHED = '@@message/OLD_MESSAGES_FETCHED';
...
export const oldMessagesFetched = (messages) => ({type: OLD_MESSAGES_FETCHED, payload: messages});
...
export const loadMessages = (messageId, oldMessages) => (dispatch) => {
    ...
    if (!oldMessages) {
        ...
        getMessages(messageId, oldMessages)
        .then(data => dispatch(oldMessagesFetched(data.Messages)))
        .catch(err => dispatch(messagesFetchingError()));
    }
